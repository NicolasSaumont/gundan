<script setup lang="ts">
  ...
  // import { orientation, resetBaseOrientation } from "~/stores/useOrientationStore";
  import { clamp, round, adjust } from "~/utils/pokemon-maths";
  import { motionProperties } from '@vueuse/motion'
  ...
  let back_img = back;
  let front_img = '~/assets/images/cards/legion/one-piece/characters/luffy5.png';
...
const defaultStiffness = 0.066
const defaultDamping = 0.25
const snapStiff = 0.01
const snapDamp = 0.06

// Définition des propriétés animées
const springRotate = motionProperties({
  x: 0,
  y: 0,
  transition: { type: 'spring', stiffness: defaultStiffness, damping: defaultDamping }
})

const springGlare = motionProperties({
  x: 50,
  y: 50,
  o: 0,
  transition: { type: 'spring', stiffness: defaultStiffness, damping: defaultDamping }
...
const springBackground = motionProperties({
  x: 50,
  y: 50,
  transition: { type: 'spring', stiffness: defaultStiffness, damping: defaultDamping }
})

// Fonction pour stopper l'interaction et réinitialiser
const interactEnd = (e, delay = 500) => {
  setTimeout(() => {
    interacting.value = false

    springRotate.transition.stiffness = snapStiff
    springRotate.transition.damping = snapDamp
    springRotate.x = 0
    springRotate.y = 0

    springGlare.transition.stiffness = snapStiff
    springGlare.transition.damping = snapDamp
    springGlare.x = 50
    springGlare.y = 50
    springGlare.o = 0

    springBackground.transition.stiffness = snapStiff
    springBackground.transition.damping = snapDamp
  ...
  const setCenter = () => {
    const rect = selectedCard.value.getBoundingClientRect(); // get element's size/position
